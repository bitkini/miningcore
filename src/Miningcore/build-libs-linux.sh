...
(
  cd ../Native/librandomx
  cp /tmp/RandomX/build/librandomx.a .
  make clean && make
) && \
mv ../Native/librandomx/librandomx.so "$OutDir"

# Build RandomARQ (tests disabled)
(
  cd /tmp
  rm -rf RandomARQ
  git clone https://github.com/arqma/RandomARQ
  cd RandomARQ
  git checkout 14850620439045b319fa6398f5a164715c4a66ce
  mkdir build && cd build
  cmake -DARCH=native -DBUILD_TESTS=OFF ..
  make
) && \
(
  cd ../Native/librandomarq
  cp /tmp/RandomARQ/build/librandomx.a .
  make clean && make
) && \
